 ...
 *
 * The review step shared by the plan modal and the setup wizard: the dates the
 * assistant proposed, one row per work item, every date still editable and every
 * row still refusable. Nothing here writes — the caller applies `approved`.
 */
...
export type TPlanReview = {
  ...
  approved: { issue_id: string; start_date: string; target_date: string }[];
};

type TRowState = { checked: boolean; start_date: string; target_date: string };
...
export function usePlanReview(assignments: TPlanAssignment[] | undefined): TPlanReview {
  ...
  // A new proposal replaces the review wholesale: everything checked, dates as proposed.
  useEffect(() => {
    const seeded: Record<string, TRowState> = {};
    list.forEach((a) => {
      seeded[a.issue_id] = { checked: true, start_date: a.start_date, target_date: a.target_date };
    });
    setState(seeded);
  }, [list]);
  ...
  const rows = useMemo<TPlanReviewRow[]>(
    () =>
      list.map((a) => ({
        ...a,
        ...(state[a.issue_id] ?? { checked: true, start_date: a.start_date, target_date: a.target_date }),
      })),
    [list, state]
  );
  ...
  const approved = useMemo(
    () =>
      rows
        ...
        .map((r) => ({ issue_id: r.issue_id, start_date: r.start_date, target_date: r.target_date })),
    [rows]
  );

  return { rows, allChecked: rows.length > 0 && rows.every((r) => r.checked), toggle, toggleAll, setDate, approved };
}
...
export const PlanReviewTable = observer(function PlanReviewTable({ plan, review }: Props) {
  const { rows, allChecked, toggle, toggleAll, setDate } = review;
  const dateInput =
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-11 text-tertiary">
        ...
        {plan.model ? ` · ${plan.model}` : ""} for {plan.undated_count} undated work item(s).
